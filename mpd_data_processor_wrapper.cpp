...
BstNode*Insert(BstNode*root,int32_t dataN){
	if (root==NULL){
		root= GetNewNode(dataN);
	}
	else if (dataN<=root->dataN){
		root->left = Insert(root->left,dataN);
	}
	else { root->right = Insert(root -> right,dataN);
	}
	return root;
}
